...
int main(int argc, char** argv)
{
...

   {
        ...
        nuc_file = "system1.txt";

        //load cross-sections and nuclides.
        load_data(nuc_file);

        //sigma = {4.9415,3.8613e-3};
        sigma = {5.0,4.0e-3};
        A = 12;
        Ao = A;
        Nd = 1.7 * 0.6022 / Ao;
        std::vector<double> sigma_xs(2,0);
        sigma_xs[0] = Nd * sigma[0];
        sigma_xs[1] = Nd * sigma[1];

        //Sig_Scat[0] = sigma_xs[0];
        //Sig_Abs[0] = sigma_xs[1];
   ...
   {
   //good oportunity to raise value error.
   //
   printf("system id error, needs to be either 0 or 1");
...

    {
...

      {
...

        {
            ...
            {
                   rxn_th = Sig_Scat[0] / sigt;
            }

            else
            {
                std::cout << "wrong system_id, only 0 or 1 is allowed" << std::endl;
        ...
        if ( anisotropic_scattering)
        {
                ...
                mu_denom = std::sqrt(std::pow(Ao,2)+2*Ao*eta+1);
                mu = (1.0 + Ao*eta) / mu_denom;
...

        {
                ...
                cosy = cosyNew;
                cosz = coszNew;
               std::cout << "A " << Ao << std::endl;
               std::cout << "E1 " << E << std::endl;
               std::cout << "eta " << eta << std::endl;
               E = E * (std::pow(Ao,2) + 1 + 2*Ao*eta) / std::pow((Ao+1),2);
...
auto f_leak_counter = count_leakage / nNeutrons;
...
std::cout << " fraction of neutrons that leaked " << f_leak_auto << std::endl;
std::cout << " fraction of neutrons that leaked, using counter " << f_leak_counter << std::endl;
...

{
  ...
  double cube = 50.0;
  XPlane.point = {cube,0.0,0.0};
  nXPlane.point = {-cube,0.0,0.0};
  YPlane.point = {0.0,cube,0.0};
  nYPlane.point = {0.0,-cube,0.0};
  ZPlane.point = {0.0,0.0,cube};
  nZPlane.point = {0.0,0.0,-cube};
