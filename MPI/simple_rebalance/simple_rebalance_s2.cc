...

{
    ...
    */
    ...
    while (tot_converged < tot_ranks)
    {
        ...
        //std::cout << "(after), from node "<<myrank << ", ls[" << i << "] = " << local_sites[i] << "and fsc " << fsc_size << std::endl;
...
void setup(const std::vector<int>& fission_sites)
{
    ...
    total_sites = fission_sites[fsc_size-1]-1;
    ...
    if (myrank == 0)
    {
        ...
        for (int i = 0; i < min_work.size() ; ++i)
        {
    
        std::cout << "(after), from node "<<myrank << ", min_work[" << i << "] = " << min_work[i] << std::endl;
