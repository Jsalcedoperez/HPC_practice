...
#define l(row,col,ncols) {(row * ncols + col);}
...
__global__ void get_dst(float *dst, float *x, float *y,float *mu_x, float *mu_y){

      int i = blockIdx.x;
      int j = threadIdx.x;

      dst[l(i,j,blockDim.x)] = (x[i] - mu_x[j]) * (x[i] - mu_x[j]);
      dst[l(i,j,blockDim.x)] += (y[i] - mu_y[j]) * (y[i] - mu_y[j]);
...
__global__ void recenter_step1(float *sum_x, float *sum_y, int *nx, int *ny, float *x, float *y, int *group, int n){
...
__global__ void recenter_step2(float *mu_x, float *mu_y, float *sum_x,float *sum_y, int *nx, int *ny){ 
     int j = threadIdx.x;
     mu_x[j] = sum_x[j] / nx[j];
     mu_y[j] = sum_y[j] / ny[j];
}
...
void kmeans(int nreps, int n, int k,
            float *x_d, float *y_d, float *mu_x_d, float *mu_y_d,int *group_d,int *nx_d,int* ny_d,float *sum_x_d, float *sum_y_d, float *dst_d){
...
void print_results(int *group, float *mu_x, float *mu_y, int , int k);
...
int main (){
  ...
  CUDA_CALL(cudaMalloc((void**) &mu_x_d,k*sizeof(int)));
  CUDA_CALL(cudaMalloc((void**) &mu_y_d,k*sizeof(int)));
  CUDA_CALL(cudaMalloc((void**) &sum_x_d,k*sizeof(int)));
  CUDA_CALL(cudaMalloc((void**) &sum_y_d,k*sizeof(int)));
  CUDA_CALL(cudaMalloc((void**) &dst_d,k*sizeof(int)));
  CUDA_CALL(cudaMalloc((void**) &group_d,n*sizeof(int)));
  ...
  CUDA_CALL(cudaMemcpy(mu_x, group, n*sizeof(int),cudaMemcpyDeviceToHost));
