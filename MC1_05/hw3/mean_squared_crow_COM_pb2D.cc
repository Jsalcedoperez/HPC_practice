...
double interpolate(double Ex, double** Macros);
...
int main()
{
...

  {
    ...
    
    {
      ...
      {
      macroXS = interpolate(E,d_macroXS);
      }
...

{
    ...
    double xs = (Macros[i][1] - Macros[i-1][1]) * (Ex - Macros[i-1][0]) / ( Macros[i][0] - Macros[i-1][0])  + Macros[i-1][1];
    return xs;
